...
(Vara.prototype.createNode = function (a, b) {
  for (var c in ((a = document.createElementNS(
  ...
  (Vara.prototype.getSVGData = function () {
    ...
    b.open("GET", this.fontSource, !0),
      (b.onreadystatechange = function () {
        4 == b.readyState &&
          200 == b.status &&
          ((a.contents = JSON.parse(b.responseText)),
          (a.characters = JSON.parse(b.responseText).c),
          a.preCreate(),
          a.createText());
      }),
      b.send(null);
        ...
        if (0 < a.breakPoints[b][f].length)
          for (var h, i, l = a.breakPoints[b][f].length, m = 0; m <= l; m++) {
            (i = a.breakPoints[b][f][m]),
              a.breakPoints[b][f][l - 1] != g.length &&
                a.breakPoints[b][f].push(g.length);
            var n =
              null == a.breakPoints[b][f][m - 1]
                ? 0
                : a.breakPoints[b][f][m - 1];
  ...
  (Vara.prototype.createText = function () {
    ...
    for (var g = 0, h = !0, k = 0; k < this.texts.length; k++) {
      ...
      var n = this.texts[k].strokeWidth,
        o =
          this.texts[k].color == null
            ? this.properties.color == null
              ? "black"
              : this.properties.color
            : this.texts[k].color,
        p =
          this.texts[k].duration == null
            ? this.properties.duration == null
              ? 2e3
              : this.properties.duration
            : this.texts[k].duration;
      this.texts[k].duration = p;
      var q = this.texts[k].id == null ? k : this.texts[k].id;
      (this.texts[k].delay =
        this.texts[k].delay == null ? 0 : this.texts[k].delay),
        (this.prevDuration += this.texts[k].delay),
        this.texts[k].fromCurrentPosition == null &&
          (this.texts[k].fromCurrentPosition = { x: !0, y: !0 }),
        (h =
          !(null != this.texts[k].y) ||
          !!(
            null == this.texts[k].fromCurrentPosition.y ||
            this.texts[k].fromCurrentPosition.y
          ));
      var r = o,
        s =
          this.texts[k].textAlign == null
            ? this.properties.textAlign
            : this.texts[k].textAlign;
      ...
      for (var u = 0, v = 0; v < this.texts[k].text.length; v++) {
        ...
        for (var z = 0, A = 0, B = 0; B < this.texts[k].text[v].length; B++) {
          ...
          var d = 0,
            E =
              this.characters[C.charCodeAt(0)] != null || " " == C
                ? " " == C
                  ? this.space.paths
                  : this.characters[C.charCodeAt(0)].paths
            ...
            E.forEach(function (a, c) {
              ...
              D.appendChild(d),
                ...
                (d.style.strokeDasharray =
                  d.getTotalLength() + " " + (d.getTotalLength() + 2)),
          ...
          "object" === _typeof(F) &&
            (F = F[C] === void 0 ? (F.global === void 0 ? 0 : F.global) : F[C]),
            D.getBBox().width < this.texts[k].minWidth &&
              (d -= (this.texts[k].minWidth - D.getBBox().width) / 2),
            D.setAttribute(
              "transform",
              "translate(" + (w - d + F) + ",0)  scale(" + a + ")"
            ),
            (w += D.getBBox().width * a + F),
            D.getBBox().width < this.texts[k].minWidth &&
              (w += this.texts[k].minWidth - D.getBBox().width);
        }
        var G = y.getBBox();
        y.setAttribute(
          "transform",
          "translate(" + n * a + "," + (-G.y + n * a) + ")"
        );
        var H = 0;
        "center" == s
          ? (H = (this.svg.getBoundingClientRect().width - G.width) / 2)
          : "right" == s &&
            (H = this.svg.getBoundingClientRect().width - G.width),
          this.setPosition(y, { x: H, y: m + this.contents.p.tf * a - z }),
          (m += f * a),
          (u += this.contents.p.tf * a + (this.contents.p.tf * a - A));
      }
      (this.texts[k].y == null || !0 == this.texts[k].fromCurrentPosition.y) &&
        this.setPosition(t, { y: e }),
        this.texts[k].fromCurrentPosition != null &&
          this.texts[k].fromCurrentPosition.y &&
          (g += this.texts[k].y == null ? 0 : this.texts[k].y),
        this.setPosition(
          t,
          { x: this.texts[k].x, y: this.texts[k].y },
          this.texts[k].fromCurrentPosition
        ),
        h && (g += u),
        this.drawnCharacters[q] != null && (q = k),
        (this.drawnCharacters[q] = {
          characters: l,
          queued: this.texts[k].queued,
          container: t,
          index: k,
        }),
        (this.texts[k].autoAnimation == null || this.texts[k].autoAnimation) &&
          this.properties.autoAnimation &&
          (b.draw(q, p),
          (this.texts[k].queued == null || this.texts[k].queued) &&
            (b.prevDuration += p));
    }
    (this.completed = !0),
  ...
  (Vara.prototype.playAll = function () {
    ...
    for (var a = 0; a < this.texts.length; a++) {
      ...
      (this.prevDuration += this.texts[a].delay),
        this.draw(c, b),
        (this.texts[a].queued == null || this.texts[a].queued) &&
          (this.prevDuration += b);
    }
  }),
  (Vara.prototype.draw = function (a, b) {
    var c = this;
    if (null == this.drawnCharacters[a])
      return (
        console.warn("ID:`" + a + "` not found. Animation skipped"),
        void console.trace()
      );
    var d =
        b === void 0 ? this.texts[this.drawnCharacters[a].index].duration : b,
      e = this.getSectionPathLength(a),
      f = 0,
      g =
        !(this.drawnCharacters[a].queued != null) ||
        this.drawnCharacters[a].queued,
  ...
  (Vara.prototype.get = function (a) {
    this;
    return null == this.drawnCharacters[a]
      ? (console.warn("ID:`" + a + "` not found."), console.trace(), !1)
      : this.drawnCharacters[a];
  }),
      ...
      setTimeout(function () {
        var c = new Date().getTime(),
          ...
          g = setInterval(function () {
            ...
            (a.style.strokeDashoffset = f + e * (d - f)),
              1 == e && clearInterval(g);
          }, e.frameRate);
      }, c);
  }),
  ...
  (Vara.prototype.analyseWidth = function () {
    var a = String.fromCharCode,
      ...
      e = [],
      f =
        null == this.characters[97]
          ? Object.keys(this.characters)[
              b(Math.random() * this.characters.length)
            ]
          : "97",
    ...
    for (var k = 0; k < this.texts.length; k++) {
      ...
      (l = "string" == typeof m.text ? [m.text] : m.text),
        (this.texts[k].text = l),
        (this.texts[k].letterSpacing =
          null == this.texts[k].letterSpacing
            ? null == this.properties.letterSpacing
              ? 0
              : this.properties.letterSpacing
            : this.texts[k].letterSpacing),
        (this.texts[k].strokeWidth =
          null == this.texts[k].strokeWidth
            ? null == this.properties.strokeWidth
              ? this.contents.p.bsw
              : this.properties.strokeWidth
            : this.texts[k].strokeWidth);
      var n =
          null == this.texts[k].breakWord
            ? null != this.properties.breakWord && this.properties.breakWord
            : this.texts[k].breakWord,
        o =
          null == m.fontSize
            ? null == this.properties.fontSize
              ? this.fontSize
        ...
        for (var w = t, y = [], z = l[v], A = 0, B = 0; B < z.length; B++)
          if (
            ("object" === _typeof(u) &&
              "object" === _typeof(u) &&
              (u =
                void 0 === u[z] ? (void 0 === u.global ? 0 : u.global) : u[z]),
            null == this.characters[z[B].charCodeAt(0)]
              ? " " == z[B]
                ? ((q = this.space.w * p), (A = w))
                : (q = this.questionMark.w * p + u)
              : ((q = this.characters[z[B].charCodeAt(0)].w * p),
                q < this.texts[k].minWidth &&
                  (q +=
                    (p *
                      (this.texts[k].minWidth -
                        this.characters[z[B].charCodeAt(0)].w)) /
                    2),
                (q += u)),
            ...
            0 == A && (n = !0);
            var C = B;
            " " == z[B] || n || (C = z.slice(0, C + 1).search(/\S+$/)),
              y.push(C),
              (w = t + w - A);
          } else (c += q), (w += q);
        s.push(y);
  ...
  (Vara.prototype.setPosition = function (a, b, c) {
    ...
    var d = a.transform.baseVal.consolidate().matrix,
      e = d.e,
      f = d.f;
    b.x != null && (c.x ? (e += b.x) : (e = b.x)),
      b.y != null && (c.y ? (f += b.y) : (f = b.y - a.getBBox().y));
